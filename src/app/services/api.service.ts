...
export class ApiService {
  ...
  postFromApi( params, apiRoute, alternativeRoute? ){
    ...
    let url = `${ this.apiUrl }${ apiRoute }.json.php`
  ...
  postToApi( params, apiRoute ){
    ...
    let url = `${ this.apiPostUrl }${ apiRoute }.post.php`
    let urlOK = this.transform( url )
  ...
  restfulGet( id, apiRoute, loginReq = true  ){

    let currentUser = JSON.parse(localStorage.getItem('currentUser'));

    let url = `${ this.apiRestful }${ apiRoute }/${ id }`

    if( loginReq ){
      url = `${ this.apiRestful }${ apiRoute }/${ id }?token=${currentUser ? currentUser.token : 'noToken'}&usn=${currentUser ? currentUser.username : 'noUser'}&usid=${currentUser ? currentUser.hcInfo.id : 'noId'}&localIp=${this.localIp}`
    }
